...
ssize_t openhook_proc_write(struct file *file, const char __user *ubuf, size_t size, loff_t *offset) 
{
	...
	if (copy_from_user(buf, ubuf, size))
		return -EFAULT ;
	// if it's file tracking problem, buf is userid that has been written in /proc/bingo
	sscanf(buf ,"%d", &userid);
	
	// if it's process kill preven probvlem, buf is pid that has been written in /proc/bingo
	//if (strcmp(buf[0],"2") == 0) sscanf(buf, "%d", &pid);
